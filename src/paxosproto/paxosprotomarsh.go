...
func (t *AcceptReply) Unmarshal(wire io.Reader) error {
	...
	t.OK = uint8(bs[4])
	t.Ballot = int32((uint32(bs[5]) | (uint32(bs[6]) << 8) | (uint32(bs[7]) << 16) | (uint32(bs[8]) << 24)))
	t.LeaseInstance = int32((uint32(bs[9]) | (uint32(bs[10]) << 8) | (uint32(bs[11]) << 16) | (uint32(bs[12]) << 24)))
	t.OriginReplica = int32((uint32(bs[13]) | (uint32(bs[14]) << 8) | (uint32(bs[15]) << 16) | (uint32(bs[16]) << 24)))
	t.PropId = int32((uint32(bs[17]) | (uint32(bs[18]) << 8) | (uint32(bs[19]) << 16) | (uint32(bs[20]) << 24)))
	return nil
}
...
func (t *Forward) New() fastrpc.Serializable {
	return new(Forward)
}
func (t *Forward) BinarySize() (nbytes int, sizeKnown bool) {
	return 0, false
}
...
func NewForwardCache() *ForwardCache {
	c := &ForwardCache{}
	c.cache = make([]*Forward, 0)
	return c
}

func (p *ForwardCache) Get() *Forward {
	var t *Forward
	p.mu.Lock()
	if len(p.cache) > 0 {
		t = p.cache[len(p.cache)-1]
...
func (p *ForwardCache) Put(t *Forward) {
	p.mu.Lock()
	p.cache = append(p.cache, t)
	p.mu.Unlock()
}
func (t *Forward) Marshal(wire io.Writer) {
	var b [8]byte
	var bs []byte
	bs = b[:8]
	tmp32 := t.ReplicaId
...
func (t *Forward) Unmarshal(wire io.Reader) error {
	var b [8]byte
	var bs []byte
	bs = b[:8]
	...
	t.PropId = int32((uint32(bs[4]) | (uint32(bs[5]) << 8) | (uint32(bs[6]) << 16) | (uint32(bs[7]) << 24)))
	t.Command.Unmarshal(wire)
	return nil
}

func (t *ForwardReply) New() fastrpc.Serializable {
	return new(ForwardReply)
}
func (t *ForwardReply) BinarySize() (nbytes int, sizeKnown bool) {
	return 0, false
}
...
func NewForwardReplyCache() *ForwardReplyCache {
	c := &ForwardReplyCache{}
	c.cache = make([]*ForwardReply, 0)
	return c
}

func (p *ForwardReplyCache) Get() *ForwardReply {
	var t *ForwardReply
	p.mu.Lock()
	if len(p.cache) > 0 {
		t = p.cache[len(p.cache)-1]
...
func (p *ForwardReplyCache) Put(t *ForwardReply) {
	p.mu.Lock()
	p.cache = append(p.cache, t)
	p.mu.Unlock()
}
func (t *ForwardReply) Marshal(wire io.Writer) {
	var b [5]byte
	var bs []byte
	bs = b[:5]
	tmp32 := t.PropId
...
func (t *ForwardReply) Unmarshal(wire io.Reader) error {
	var b [5]byte
	var bs []byte
	bs = b[:5]
	...
	t.PropId = int32((uint32(bs[0]) | (uint32(bs[1]) << 8) | (uint32(bs[2]) << 16) | (uint32(bs[3]) << 24)))
	t.OK = uint8(bs[4])
