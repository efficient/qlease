...
func (t *ProposeLease) New() fastrpc.Serializable {
	return new(ProposeLease)
}
func (t *ProposeLease) BinarySize() (nbytes int, sizeKnown bool) {
	return 0, false
}
...
func NewProposeLeaseCache() *ProposeLeaseCache {
	c := &ProposeLeaseCache{}
	c.cache = make([]*ProposeLease, 0)
	return c
}

func (p *ProposeLeaseCache) Get() *ProposeLease {
	var t *ProposeLease
	p.mu.Lock()
	if len(p.cache) > 0 {
...
func (p *ProposeLeaseCache) Put(t *ProposeLease) {
	p.mu.Lock()
	p.cache = append(p.cache, t)
	p.mu.Unlock()
}
func (t *ProposeLease) Marshal(wire io.Writer) {
	var b [10]byte
	var bs []byte
	bs = b[:4]
	tmp32 := t.ReplicaId
	...
	wire.Write(bs)
	bs = b[:]
	alen1 := int64(len(t.Updates))
	if wlen := binary.PutVarint(bs, alen1); wlen >= 0 {
		wire.Write(b[0:wlen])
	}
	for i := int64(0); i < alen1; i++ {
		t.Updates[i].Marshal(wire)
	}
}

func (t *ProposeLease) Unmarshal(rr io.Reader) error {
	var wire byteReader
	var ok bool
	...
	t.Updates = make([]qleaseproto.LeaseMetadata, alen1)
	for i := int64(0); i < alen1; i++ {
		t.Updates[i].Unmarshal(wire)
	}
	return nil
}
